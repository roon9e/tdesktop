...
bool ResolveUsernameOrPhone(
	...
	if (domainParam == u"oauth"_q) {
		...
		if (!token.isEmpty()) {
			UrlAuthBox::ActivateUrl(
				controller->uiShow(),
				&controller->session(),
				u"owpg://resolve?domain=oauth&startapp="_q
	...
	// Fix teleram.ru/s/username links.
...
bool ResolveOAuth(
	...
	UrlAuthBox::ActivateUrl(
		controller->uiShow(),
		&controller->session(),
		u"owpg://oauth?token="_q + qthelp::url_encode(token),
		context);
	return true;
}
...
QString TryConvertUrlToLocal(QString url) {
	...
	auto subdomainMatch = regex_match(u"^(https?://)?([a-zA-Z0-9\\_]+)\\.teleram\\.ru(/\\d+)?/?(\\?.+)?"_q, url, matchOptions);
	if (subdomainMatch) {
		...
		if (name.size() > 1 && name != "www") {
			...
			return result.startsWith("owpg://resolve?domain=")
				? result
				: url;
		}
	}
	//auto telegramMeMatch = regex_match(u"^(https?://)?(www\\.)?(telegram\\.(me|dog)|t\\.me)/(.+)$"_q, url, matchOptions);
	auto telegramMeMatch = regex_match(u"^(https?://)?(www\\.)?teleram\\.ru/(.+)$"_q, url, matchOptions);
	...
	if (telegramMeMatch) {
		...
		if (const auto phoneMatch = regex_match(u"^\\+([0-9]+)(\\?|$)"_q, query, matchOptions)) {
			...
			const auto type = regex_match(u"^[a-fA-F0-9]{6}^"_q, bg)
				...
				? "gradient"
				: "slug";
			return u"owpg://bg?"_q + type + '=' + bg + (params.isEmpty() ? QString() : '&' + params);
			...
			auto result = u"owpg://newbot?manager="_q + url_encode(manager);
			if (!username.isEmpty()) {
			...
			if (params.indexOf("boost", 0, Qt::CaseInsensitive) >= 0
				...
				return u"owpg://boost?channel="_q + channel;
			}
			const auto base = u"owpg://privatepost?channel="_q + channel;
			auto added = QString();
			...
			if (params.indexOf("boost", 0, Qt::CaseInsensitive) >= 0
				&& params.toLower().split('&').contains(u"boost"_q)) {
				return u"owpg://boost?domain="_q + domain;
			} else if (domain == u"boost"_q) {
				if (const auto domainMatch = regex_match(u"^/([a-zA-Z0-9\\.\\_]+)(/?\\?|/?$)"_q, usernameMatch->captured(2))) {
					return u"owpg://boost?domain="_q + domainMatch->captured(1);
				} else if (params.indexOf("c=", 0, Qt::CaseInsensitive) >= 0) {
					return u"owpg://boost?"_q + params;
				}
			}
			const auto base = u"owpg://resolve?domain="_q + url_encode(usernameMatch->captured(1));
			auto added = QString();
...
bool IsMiniAppUrl(const QString &url) {
	...
	const auto prefix = u"owpg://resolve?"_q;
...
[[nodiscard]] InternalLinkCheckResult InternalLinkCheck(const QString &url) {
	...
	const auto command = base::StringViewMid(urlTrimmed, u"owpg://"_q.size());
